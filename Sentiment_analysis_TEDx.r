...
rm(list=ls())
setwd("/Users/pivanova/Documents/Projects/TEDx")
data <- read.csv("ted_main.csv",na.strings = c(""," ","NA"), stringsAsFactors = FALSE)
transcript <- read.csv("transcripts.csv",na.strings = c(""," ","NA"), stringsAsFactors = FALSE)

# 1. Summarize variables class for each data.frame
dataclass <- data.frame(names(data),rapply(data,class))
colnames(dataclass) <- c("Varname","Varclass")
(length(unique(data$name))) #[1] 2550
(length(unique(data$url))) #[1] 2550

trclass <- data.frame(names(transcript),rapply(transcript,class))
colnames(trclass) <- c("Varname","Varclass")
(length(unique(transcript$url))) #[1] 2464 # There are duplicates
transcript$url[duplicated(transcript$url)==T]
...
(which(duplicated(transcript$url)==T)) #[1] 1115 1116 1117

# 2. Remove duplicates
transcript <- transcript[-(which(duplicated(transcript$url)==T)),]

# 3. Identify matches and differences
url <- list()
url$both <- intersect(data$url, transcript$url)
(length(url$both)) #[1] 2464
url$data <- setdiff(data$url, transcript$url)
(length(url$data)) #[1] 86

# 4. Merge data.frames
dd <- merge(transcript, data, by="url")
...
p <- ggplot(data = head(processed_tags, 10), aes(processed_tags, Freq, fill = processed_tags)) +
...
top_com <- arrange(dd, comments)
keeps <- c("title", "main_speaker", "views", "comments")
top_com <- subset(top_com, select = keeps)
top_com <- top_com %>%
...
top_com <- arrange(dd, desc(comments))
top_com <- subset(top_com, select = keeps)
top_10_talks <- top_com[0:10,]
top_10_talks

p2 <- ggplot(data = top_10_talks, aes(main_speaker, comments, fill = comments)) +
  geom_bar(position = "dodge", stat = "identity") +
...
talks_view <- arrange(dd, views)
keeps <- c("title", "main_speaker", "views", "comments")
talks_view <- subset(talks_view, select = keeps)
talks_view <- arrange(talks_view, desc(views))
ten_talks_view <- talks_view[0:10,]
ten_talks_view[1,]

p3 <- ggplot(data = ten_talks_view, aes(main_speaker, views, fill = views)) +
  geom_bar(position = "dodge", stat = "identity") +
...
talks_rat <- arrange(dd, desc(dis_quo))
keeps <- c("title", "main_speaker", "views", "comments", "dis_quo")
talks_rat <- subset(talks_rat, select = keeps)
top_10_talks_r <- talks_rat[0:10,]
top_10_talks_r[1,]

p4 <- ggplot(data = top_10_talks_r, aes(main_speaker, dis_quo, fill = dis_quo)) +
  geom_bar(position = "dodge", stat = "identity") +
  geom_text(aes(label = comments), vjust = 1.6, color = "white", size = 3) +
...
# 1. Sentiment analysis on video "Enough with the fear of fat"
